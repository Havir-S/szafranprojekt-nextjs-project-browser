...
function EditProject({currentEditingProject, setCurrentEditingProject, openFolder, toggleEditor}) {
  ...
  return (
    <div className='absolute px-3 top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 w-3/5 h-5/6 border-8 border-sky-800 rounded-lg shadow-2xl bg-gray-200'>
        <div className='flex py-1 text-3xl justify-between items-center'>
            ...
            <div className='flex gap-5'>
                ...
                <button onClick={() => {openFolder(`${currentEditingProject.project_disk}:\\szafranprojekt\\${currentEditingProject.project_number}-${currentEditingProject.project_name}`)}} className='border-4 cursor-pointer rounded-lg py-2 font-bold px-5 bg-sky-200 border-sky-400 hover:bg-sky-300'>Otwórz folder</button>
